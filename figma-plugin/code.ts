...
import {
  ...
  CreateFramePayload,
  CreateTextPayload,
  isCreateFrameCommand,
  isCreateTextCommand,
  validateCreateFramePayload,
  validateCreateTextPayload,
} from './types';
 ...
 */
async function createFrame(payload: CreateFramePayload): Promise<PluginResponse> {
  try {
 ...
 */
async function createText(payload: CreateTextPayload): Promise<PluginResponse> {
  try {
 ...
 */
async function handleCommand(command: FigmaCommand): Promise<PluginResponse> {
 ...
 */
figma.ui.onmessage = async (msg: unknown): Promise<void> => {
  try {
    ...
    let command: FigmaCommand;

    if (message.type === 'create_frame') {
      command = {
        type: 'create_frame',
        payload: message.payload as CreateFramePayload,
      };
    } else if (message.type === 'create_text') {
      command = {
        type: 'create_text',
        payload: message.payload as CreateTextPayload,
      };
    } else {
