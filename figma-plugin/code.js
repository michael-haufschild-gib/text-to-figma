    ...
    try {
        ...
        switch (type) {
            ...
            case 'set_corner_radius': {
                try {
                    ...
                    if (!('cornerRadius' in node)) {
                        throw new Error(`Node ${payload.nodeId} does not support corner radius (type: ${node.type})`);
                    }
                    if (payload.radius !== undefined) {
                        if ('cornerRadius' in node && typeof node.cornerRadius !== 'symbol') {
                            node.cornerRadius = payload.radius;
            ...
            case 'set_layout_properties': {
                ...
                if (payload.itemSpacing !== undefined)
                    node.itemSpacing = payload.itemSpacing;
                if (payload.padding !== undefined) {
                    node.paddingLeft =
                        node.paddingRight =
                            node.paddingTop =
            ...
            case 'apply_effects': {
                ...
                for (const effect of payload.effects) {
                    if (effect.type === 'DROP_SHADOW' || effect.type === 'INNER_SHADOW') {
                        ...
                        effects.push({
                            type: effect.type,
                            color: Object.assign(Object.assign({}, rgb), { a: opacity }),
            ...
            case 'create_path': {
                try {
                            ...
                            switch (cmd.type) {
                                case 'M':
                                    ...
                                    if (!isFinite(cmd.x) || !isFinite(cmd.y)) {
                                        throw new Error(`M command at index ${i} has invalid coordinates (x=${cmd.x}, y=${cmd.y}). Coordinates must be finite numbers.`);
                                    }
                                ...
                                case 'L':
                                    ...
                                    if (!isFinite(cmd.x) || !isFinite(cmd.y)) {
                                        throw new Error(`L command at index ${i} has invalid coordinates (x=${cmd.x}, y=${cmd.y}). Coordinates must be finite numbers.`);
                                    }
                                ...
                                case 'C':
                                    if (typeof cmd.x1 !== 'number' ||
                                        ...
                                        typeof cmd.x !== 'number' ||
                                        typeof cmd.y !== 'number') {
                                        throw new Error(`C command at index ${i} missing required coordinates (x1, y1, x2, y2, x, y). Got: x1=${cmd.x1}, y1=${cmd.y1}, x2=${cmd.x2}, y2=${cmd.y2}, x=${cmd.x}, y=${cmd.y}`);
                                    }
                                    if (!isFinite(cmd.x1) || !isFinite(cmd.y1) ||
                                        !isFinite(cmd.x2) || !isFinite(cmd.y2) ||
                                        !isFinite(cmd.x) || !isFinite(cmd.y)) {
                                        throw new Error(`C command at index ${i} has invalid coordinates. All values must be finite numbers.`);
                                    }
                                ...
                                case 'Q':
                                    if (typeof cmd.x1 !== 'number' ||
                                        ...
                                        typeof cmd.x !== 'number' ||
                                        typeof cmd.y !== 'number') {
                                        throw new Error(`Q command at index ${i} missing required coordinates (x1, y1, x, y). Got: x1=${cmd.x1}, y1=${cmd.y1}, x=${cmd.x}, y=${cmd.y}`);
                                    }
                                    if (!isFinite(cmd.x1) || !isFinite(cmd.y1) ||
                                        !isFinite(cmd.x) || !isFinite(cmd.y)) {
                                        throw new Error(`Q command at index ${i} has invalid coordinates. All values must be finite numbers.`);
                                    }
                    ...
                    console.log(`[create_path] Generated path data: ${trimmedPath.substring(0, 200)}${trimmedPath.length > 200 ? '...' : ''}`);
            ...
            case 'connect_shapes': {
                ...
                if (method === 'POSITION_OVERLAP' || method === 'UNION') {
                    ...
                    else if (payload.targetAnchor.includes('BOTTOM') &&
                        payload.sourceAnchor.includes('TOP')) {
                        sourceY -= overlap;
                    }
                    else if (payload.targetAnchor.includes('LEFT') &&
                        payload.sourceAnchor.includes('RIGHT')) {
                        sourceX += overlap;
                    }
                    else if (payload.targetAnchor.includes('RIGHT') &&
            ...
            case 'add_gradient_fill': {
                ...
                const stops = payload.stops.map((stop) => ({
                    ...
                    color: Object.assign(Object.assign({}, hexToRgb(stop.color)), { a: stop.opacity || 1 })
                }));
                const gradient = {
                    type: gradientType,
                    gradientStops: stops,
            ...
            case 'create_boolean_operation': {
                ...
                const nodes = payload.nodeIds.map((id) => getNode(id)).filter((n) => n);
                if (nodes.length < 2) {
            ...
            case 'set_instance_swap': {
                ...
                const oldComponentId = (_a = instance.mainComponent) === null || _a === void 0 ? void 0 : _a.id;
            ...
            case 'add_layout_grid': {
                ...
                const grid = {
                    ...
                    color: payload.color
                        ? Object.assign(Object.assign({}, hexToRgb(payload.color)), { a: 0.1 }) : { r: 1, g: 0, b: 0, a: 0.1 },
