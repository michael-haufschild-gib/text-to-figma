...
export async function setVisible(
  ...
  const response = await bridge.sendToFigma<{ success: boolean; error?: string }>('set_visible', {
  ...
  return {
    ...
    visible: validated.visible,
    cssEquivalent,
    message: validated.visible ? 'Showed node' : 'Hid node'
  };
}
